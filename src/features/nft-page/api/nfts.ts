...
export async function getAllNfts() {
  if (cache.has(NFT_CACHE_KEY) && cache.has(POAP_CACHE_KEY)) {
    return {
      nft: (await cache.get(NFT_CACHE_KEY)) as [NormalizedPoapData],
      poap: (await cache.get(POAP_CACHE_KEY)) as NormalizedPoapData,
    };
  }
  ...
  return {
    nft: nftCollection as unknown as [NormalizedPoapData],
    poap: poapCollection,
  };
}

export type GetAllNfts = Awaited<ReturnType<typeof getAllNfts>>;
